function gscroll() {
  ...
  function initialize(d, i) {
    this.__gscroll__ = {index: i, active: false};
  }

  function activate() {
    if (!this.__gscroll__.active) {
      this.__gscroll__.active = true;
      dispatchEvent({type: "activate"}, this.__gscroll__.index);
    }
  }

  function deactivate() {
    if (this.__gscroll__.active) {
      this.__gscroll__.active = false;
      dispatchEvent({type: "deactivate"}, this.__gscroll__.index);
    }
  }
  ...
  gscroll.size = function(_) {
    return arguments.length ? (size = [+_[0], +_[1]], resize(), gscroll) : size;
  };

  gscroll.scrollRatio = function(_) {
    return arguments.length ? (scrollRatio = +_, resize(), gscroll) : scrollRatio;
  };

  gscroll.fontSize = function(_) {
    return arguments.length ? (fontSize = +_, resize(), gscroll) : fontSize;
  };
